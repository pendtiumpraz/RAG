...
|---------|------|-----------|
| 2026-09-20 | 07 | **Graf memory yang padat kini terbaca.** Dilaporkan: graf "Asisten PDP" menumpuk di tengah. Diukur, bukan diduga — PDP berderajat rata-rata 19,2 (2.165 sisi, 1.655 di antaranya kemiripan) sementara hr 4,7 dan Kepatuhan 5,1; hanya yang padat yang runtuh. TIGA sebab terpisah: (1) pegas tak dinormalisasi derajat — node berderajat 19 menerima 19 tarikan melawan repulsi yang bertutup 3,5, sehingga hub (catatan MOC) duduk 4,5px dari tetangganya alias tertimbun; (2) konstanta tata letak tetap untuk 40 maupun 1.000 node, padahal panjang ideal sisi sebanding √(luas/N); (3) ambang label MUTLAK `deg >= 6` yang di graf padat berlaku untuk hampir semua node → 123 label tergambar bertumpuk. Fisikanya dipindah ke `graf-tata.ts` supaya BISA diukur tanpa kanvas, lalu diperbaiki: pegas dibagi √derajat, konstanta tumbuh bersama N (40 node tetap seperti dulu), sisi kemiripan dibatasi top-k per node hanya bila grafnya memang padat (wikilink tak pernah dipangkas), label berdasar peringkat + penyaring tumpang-tindih, plus tombol "pas ke layar" dan saklar kepadatan. Diukur pada data PDP asli: median jarak antar-node 29,9px → 80,1px, label serentak 123 → 8, hub 4,5px → 45px. 9 tes penjaga baru yang MENJALANKAN simulasinya |
| 2026-09-20 | 07 | 🔴 **Satu byte NUL membatalkan seluruh dokumen di ingest — diperbaiki.** Terbukti pada korpus Kepatuhan: "55 Tahun 2026.pdf" (juklak 123 halaman) diekstrak jadi 156.111 karakter yang memuat 2.506 NUL; Postgres menolak NUL di kolom text dengan MENGGAGALKAN insert, jadi 277 potongan batal sekaligus dan dokumennya TIDAK masuk — padahal berkas aslinya sudah tersimpan di blob dan kuota penyimpanannya sudah terpakai. Galatnya menyesatkan ("Failed query: insert into documents …") sehingga terbaca seperti gangguan basis data, bukan satu PDF yang perlu dibersihkan. `teks-aman.ts` membuang NUL, kendali C0/C1 lain, dan surrogate lepas — tab/baris baru DIPERTAHANKAN karena pemotong mengenali tabel dari tab. Dipasang di `ingest()`, satu-satunya jalur yang dilewati semua cara dokumen masuk, dan SEBELUM sidik isi dihitung supaya sidik dan isi yang disimpan berasal dari teks yang sama. 4 tes penjaga baru; tiga penjaga lama yang memaku ejaan `input.text` dilonggarkan jadi menguji URUTANNYA |
| 2026-09-20 | 07 | **Korpus Kepatuhan masuk produksi**: KB "Kepatuhan" + chatbot "Kepatuhan" (baru), 8 PDF peraturan & surat edaran di blob (uploaded_files lengkap, 25,9 MB), 925 potongan. 7 dari 8 dokumen terbaca penuh; "216_SE_2024.pdf" adalah PDF HASIL PINDAIAN — halaman 2–17 hanya memuat watermark (36 karakter identik per halaman), jadi yang terindeks cuma halaman sampulnya (2 potongan). Pipeline ini tak punya OCR, jadi dokumen itu praktis tak bisa dijawab; sisanya terbukti hidup lewat retrieval (mis. ROPA → "evaluasi minimal 2 tahun sekali"). Memory Agent dijalankan lewat memory-worker |
...
| 2026-09-07 | 07 | **Korpus Regulasi PDP masuk produksi**: KB "Regulasi PDP" + chatbot "Asisten PDP" (baru), 67 dokumen · 1.863 potongan · berkas asli di blob (uploaded_files lengkap, folder terisi per subfolder 01-naskah…05-alat), lewat service yang sama dengan rute unggah. 4 "gagal" semuanya berkas kunci Word ~$ (sampah; 2 yang telanjur tersimpan dibersihkan dari blob+DB). E2E terbukti: /api/chat produksi menjawab dengan sitasi kamus-istilah.docx |
| 2026-09-07 | 07 | 🔴 **Kunci Sumopod DIBLOKIR penyedianya** — chat produksi mati (model aktif vps:qwen3.7, cadangan model yang sama → failover tak berdaya). Dipulihkan: model aktif dikembalikan ke deepseek-v4-flash (satu-satunya kunci hidup milik tenant; diuji menjawab "Jakarta"). Cadangan vps:qwen3.7 DIBIARKAN — kembali berarti begitu kunci di-unblock. Kartu a-memory-failover lahir dari kejadian ini: pipeline memory belum ikut failover |
