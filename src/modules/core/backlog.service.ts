...
const AGENT: SeedItem[] = [
  ...
  { key: 'a-chat-sessions', track: 'agent', dimension: 'uiux', size: 'S', priority: 'P2',
    title: 'Daftar & ganti riwayat sesi di konsol Chat DASBOR',
    why: 'Percakapan lama hanya bisa DIBACA lewat Conversations, dan dibaca saja — tak bisa dilanjutkan dari tempat mengetik. Pemilik chatbot yang sedang menguji jawaban justru paling sering ingin kembali ke sesi kemarin lalu bertanya susulan.' },
  ...
  { key: 'a-abuse-cost', track: 'agent', dimension: 'launch', size: 'M', priority: 'P2',
    title: 'Penolakan tetap memakan kuota — penyalahgunaan masih berbiaya',
    why: 'DITEMUKAN 31 Jul 2026 saat mengukur a-moderation. Kabar baiknya: grounding ketat sudah menahan SELURUH penyalahgunaan isi — 8 dari 8 permintaan di luar korpus (puisi, kode, terjemahan, resep, nasihat saham) ditolak dengan benar di kedua bahasa. Jadi lapis moderasi konten tak dibangun; menambahnya berarti menambah tempat baru untuk salah tanpa mengurangi apa pun. Yang TIDAK tertutup adalah biayanya: tiap penolakan tetap menjalankan retrieval penuh, memanggil model penuh, dan dihitung satu pesan di usage_counters. Seribu pesan sampah berarti seribu pesan dari kuota bulanan pelanggan dan seribu panggilan model yang mereka bayar. Pembatas laju (per chatbot + per IP) membatasi KECEPATANNYA, bukan totalnya. Yang perlu dipikirkan: apakah penolakan layak dihitung lebih murah, atau justru harus dihitung penuh supaya pelanggan melihat siapa yang membebani widget-nya. Ini keputusan produk lebih dulu, bukan soal kode — dan menambal salah satunya tanpa memutuskan yang lain akan menyembunyikan masalahnya di kolom yang berbeda.' },
  ...
  { key: 'a-lexical-dead', track: 'agent', dimension: 'agentic', size: 'S', priority: 'P0',
    title: 'Kaki leksikal MATI untuk hampir semua pertanyaan alami',
    why: 'DITEMUKAN & DIBUKTIKAN 31 Jul 2026 lewat eval, bukan dari dugaan. `plainto_tsquery` menggabungkan seluruh kata dengan AND, dan karena konfigurasinya `simple` tak ada stopword yang dibuang — jadi "berapa", "siapa", "yang", "apa", "dan" ikut jadi kata WAJIB. Terukur di korpus produksi: "berapa NPWP perusahaan" → tsquery `berapa & npwp & perusahaan` → 0 potongan; "siapa notaris yang membuat akta pendirian" → 0; "apa kode dan nama KBLI perusahaan" → 0. Sementara "NPWP" saja → 3 potongan. Artinya pada hampir setiap pertanyaan yang ditulis manusia, kaki leksikal mengembalikan KOSONG dan penggabungan peringkat jatuh ke vektor murni. Hybrid search yang dijual tiga kaki sebenarnya berjalan satu setengah. Akibat nyatanya sudah terlihat: pertanyaan NPWP dijawab "tidak ada di dokumen" walau angkanya ADA di korpus — potongan yang memuatnya tak pernah terambil karena justru kaki leksikal yang seharusnya menangkap istilah persis. Ini pula yang menghalangi hal yang paling dijual produk ini: nomor kontrak, nama orang, kode pasal. Jalan keluar: bangun tsquery ber-OR dari kata isi (buang kata tanya), atau pakai `websearch_to_tsquery`, dengan ts_rank_cd tetap memberi nilai lebih tinggi pada potongan yang mencocoki lebih banyak istilah. WAJIB diukur sebelum & sesudah dengan `npm run eval` — garis dasarnya sudah tersimpan.' },
  ...
  /* ── D18 · modul Analisis (15 Sep 2026) ─────────────────────────────
     Enam kartu di bawah adalah pecahan satu keputusan (architecture-
     decisions.md D18): policy review & contract review yang hasilnya
     dokumen resmi, bukan jawaban chat. Urutannya disengaja — tiap kartu
     selesai sendiri dan kartu berikutnya berdiri di atasnya:
     potong-struktur → template → gerbang → agen → form → ekspor.
     Prasyarat yang TAK ada di daftar ini karena sudah punya kartunya:
     a-lexical-dead (P0) — analisis yang mengutip pasal yang salah lebih
     buruk daripada tak ada analisis. */

  { key: 'a-potong-struktur', track: 'agent', dimension: 'agentic', size: 'M', priority: 'P1',
    title: 'Jalur struktural di metadata potongan — pasal/ayat/klausul ikut sampai ke sitasi',
    why: 'PRASYARAT D18 yang berguna sendiri. Pemotong SUDAH sadar makna: ia memilih batas paragraf lalu awal Pasal/BAB/1.2 sebagai titik potong, dan mengulang kepala tabel di potongan lanjutan — tapi hasilnya tetap string[] telanjang. Tiga celah yang untuk chat hanya kurang rapi tapi untuk vonis kepatuhan fatal: (1) potongan tak membawa jalur strukturalnya, jadi sitasi hanya bisa menyebut nama berkas + nomor potongan, bukan "UU PDP Pasal 20 ayat (1)"; (2) Pasal yang lebih panjang dari 800 karakter terbelah, dan potongan lanjutannya TIDAK mengulang "Pasal 20" seperti kepala tabel diulang — potongan yang memuat ayat (3) tak tahu lagi ia ayat dari pasal mana; (3) ayat "(2)", huruf "a.", dan "Klausul 7" kontrak belum dikenali sebagai batas. Yang dibutuhkan: pemotong mengembalikan {teks, jalur} dengan jalur "BAB III › Pasal 20 › ayat (1)" yang dihitung dari judul terakhir yang dilewati, mengulang judul pasal di potongan lanjutan, dan jalur itu disimpan di metadata potongan sehingga retrieval, sitasi chat, dan temuan analisis menyebutnya apa adanya. WAJIB diukur `npm run eval` sebelum & sesudah pada korpus PDP yang sudah ada (67 dokumen), lalu korpus itu di-ingest ulang — potongan lama tak punya jalur.' },

  { key: 'a-analisis-template', track: 'agent', dimension: 'feature', size: 'M', priority: 'P1',
    title: 'Template-sebagai-data: analysis_templates + kepala analyses + temuan analysis_findings',
    why: 'Fondasi data D18, tanpa satu pun perilaku yang terlihat pengguna — sengaja, supaya migrasinya kecil dan bisa diperiksa sendiri. Migrasi 0055 (guarded/idempotent, dijalankan dengan peran admin, RLS + kebijakan untuk ketiga tabel, tanpa FK, semua ber-deleted_at): `analysis_templates` per tenant (key, nama, versi, schema JSONB yang mendeskripsikan bagian & field beserta tipe/label/petunjuk prompt, is_system) — pola persis document_categories: bawaan sistem di-seed, tenant boleh menyunting salinannya, tiap suntingan menaikkan versi; `analyses` = KEPALA dengan kolom tetap untuk yang harus dicari/dihitung (tenant, chatbot, template + template_version, status queued|running|draft|final|failed, subject_file_id ke uploaded_files, model, requested_by, version, error) plus `fields` JSONB untuk isi menurut template; `analysis_findings` = satu baris per klausul/kewajiban dengan kolom tetap universal — verdict enum (sesuai|bertentangan|tak_diatur|perlu_cek_manusia), severity (tinggi|sedang|rendah), clause_ref, ringkasan, rekomendasi, `citations` JSONB ber-GIN — plus `extra` JSONB untuk field khas template. Dua template sistem di-seed: "Tinjauan Kepatuhan Kebijakan" (kebijakan internal vs regulasi) dan "Tinjauan Kontrak". Ditambah repository + service + rute master-data (CRUD, trashed, restore) dan halaman template ala kategori dokumen. template_version di tiap analisis adalah yang menjamin analisis lama tetap dirender dengan bentuk saat ia dibuat.' },

  { key: 'a-analisis-gerbang', track: 'agent', dimension: 'feature', size: 'S', priority: 'P1',
    title: 'Gerbang & kuota Analisis: flag per chatbot, palet perintah /, satu analisis = satu satuan kuota',
    why: 'Analisis adalah KEMAMPUAN per chatbot (D18), bukan fitur global — chatbot HR tak perlu tombol tinjau kontrak. Migrasi 0056: kolom `analysis_enabled` di chatbots (default false), saklar di drawer chatbot, dan palet perintah `/` di chat yang hanya muncul bila flag menyala. Yang menegakkan WAJIB server: rute chat dan API v1 menolak `/analyze` untuk chatbot yang flag-nya mati — UI hanya menyembunyikan tombol, dan tombol yang disembunyikan bukan pengaman. Kuota: satu analisis adalah puluhan panggilan LLM, jadi ia tak boleh dihitung sebagai satu pesan; tambahkan satuan `analyses` di limits.ts + paket, angkanya bisa ditimpa superadmin seperti kuota lain, tenant is_platform tetap kebal, superadmin sebagai peran TIDAK kebal. `/analyze <template> [judul]` mengacu ke berkas unggahan (bukan menempel isinya — batas 4.000 karakter pesan tetap berlaku), menulis baris `analyses` berstatus queued, memicu job, dan membalas di chat bahwa analisis diantrekan beserta tautan ke halamannya. Mesin yang mengerjakan antrean itu kartu a-analisis-agen.' },

  { key: 'a-analisis-agen', track: 'agent', dimension: 'agentic', size: 'L', priority: 'P1',
    title: 'Agen analisis: job per dokumen subjek, satu pass LLM per unit, vonis enum yang divalidasi keras',
    why: 'Inti D18 dan yang paling berat. Handler job di core/jobs.ts + pekerja VPS (pola memory-worker, karena satu analisis melewati 300 detik lambda) yang: (1) membaca dokumen subjek dari uploaded_files dan memotongnya per klausul/kewajiban memakai jalur struktural dari a-potong-struktur; (2) untuk tiap unit menarik konteks dari KB chatbot lewat retrieval 3 kaki yang sama dengan chat; (3) meminta model mengeluarkan JSON temuan yang divalidasi keras — verdict WAJIB salah satu dari sesuai|bertentangan|tak_diatur|perlu_cek_manusia, sitasi WAJIB menunjuk potongan yang benar-benar dikirim (pola citation enforcement di guardrails), field template divalidasi terhadap schema-nya; (4) menyimpan per unit begitu jadi, bukan di akhir, supaya run yang mati di menit ke-8 tak membuang tujuh menit pertama. Aturan yang tak boleh dilanggar: bukti lemah ⇒ perlu_cek_manusia, bukan tebakan; balasan kosong/cacat DIHITUNG (pola distillKosong/distillCacat) dan tampil di ringkasan run; failover ke modelCadangan() dipasang sejak awal, jangan mengulang cerita a-memory-failover; status running→draft, gagal→failed dengan sebabnya tertulis. Ukuran L karena harus DIBUKTIKAN, bukan diklaim: satu kebijakan internal ditinjau terhadap 67 dokumen regulasi PDP di produksi, dan tiap vonis bertentangan diperiksa manusia — golden set kecil itu yang menentukan apakah perlu_cek_manusia muncul secukupnya atau terlalu sering.' },

  { key: 'a-analisis-form', track: 'agent', dimension: 'feature', size: 'M', priority: 'P1',
    title: 'Halaman Analisis + form editable yang dibangun otomatis dari template',
    why: 'Halaman /analisis (CRUD satu halaman, laci kanan 400px, sesuai aturan) mendaftar analisis per chatbot dengan status & rangkuman vonis, dan detailnya adalah form per field yang DIBANGUN dari schema analysis_templates (tipe teks/paragraf/pilihan/tabel temuan) — bukan form tulisan tangan per template, karena template baru yang butuh form baru berarti fiturnya bukan template-sebagai-data. Temuan disunting per baris: vonis (pilihan enum), tingkat, ringkasan, rekomendasi; sitasi tampil sebagai chip amber yang membuka potongan sumbernya — bahasa visual RAG yang sama dengan chat. Tiap simpan menaikkan `version` + mencatat updated_by supaya cap asal-usul ekspor bisa jujur menyebut siapa mengubah apa; final = terkunci, kembali ke draft hanya oleh admin. Trashed/restore wajib seperti resource lain. Yang paling mudah hilang pada form yang dibangun dinamis adalah LABEL-nya — tes a11y yang sudah ada harus menjangkau halaman ini.' },

  { key: 'a-analisis-ekspor', track: 'agent', dimension: 'feature', size: 'M', priority: 'P1',
    title: 'Ekspor DOCX (pustaka docx) + PDF dari satu model isi, berkop tenant, bercap asal-usul',
    why: 'Yang dijual modul Analisis adalah dokumen yang bisa diserahkan ke atasan atau legal, dan itu berarti DOCX yang terbuka rapi di Word — bukan HTML yang disimpan ulang. Dependensi sekarang hanya punya pptxgenjs + qrcode; belum ada pustaka DOCX maupun PDF. Bangun satu MODEL ISI (judul, meta, bagian menurut template, tabel temuan, sitasi sebagai catatan kaki) dari kepala + temuan + template, lalu dua penyaji: `docx` (heading, numbering, tabel, footnote, header/footer) dan PDF murni server-side (pdf-lib/pdfkit) — TANPA headless browser, Chromium tak muat di lambda. Kop dari branding tenant (logoUrl, nama), tipografi dari design system. Tiap berkas dicap asal-usul: nomor analisis, versi, template + versinya, model, waktu, penyunting, dan kalimat "dibuat dengan bantuan AI, ditinjau oleh …" — dokumen tanpa cap itu akan beredar sebagai pendapat hukum. Konversi DOCX→PDF lewat LibreOffice DITOLAK untuk Vercel (boleh kelak di VPS bila PDF harus identik piksel). Rute ekspor mengalirkan berkas dan dicatat di audit; dibuktikan dengan membaca ulang hasilnya lewat mammoth (DOCX) dan pdf-parse (PDF) yang sudah ada di dependensi — berkas yang terbuat belum tentu terbaca.' },
];
