...
const AGENT: SeedItem[] = [
  ...
  { key: 'a-policy-adherence', track: 'agent', dimension: 'agentic', size: 'M', priority: 'P1',
    title: 'Kepatuhan bahasa & penolakan berlebih — dua pelanggaran yang TERUKUR',
    why: 'Ditemukan oleh a-answer-policy-eval saat dijalankan sungguhan (DeepSeek V4 Flash, korpus produksi, 31 Jul 2026), bukan dari dugaan. Anti-karangan LULUS 100% — lima dari lima pertanyaan tanpa jawaban ditolak dengan benar. Dua hal lain TIDAK. (1) BAHASA: tiga dari dua belas jawaban memakai bahasa yang salah — pertanyaan berbahasa Inggris dijawab bahasa Indonesia. Arahannya sudah terpasang dan sampai ke prompt (LANGUAGE di answer-policy.ts, disisipkan lewat policyDirectives di chatTurn), jadi ini BUKAN cacat pemasangan seperti temperature dulu: modelnya yang tertarik mengikuti bahasa dokumen alih-alih bahasa penanya, karena seluruh konteksnya berbahasa Indonesia. Kemungkinan jalan keluar: menegaskan arahan bahasa di akhir prompt (bukan di awal, yang lebih mudah tenggelam), atau memeriksa bahasa keluaran lalu meminta ulang sekali. (2) MENOLAK BERLEBIH: pertanyaan NPWP ditolak padahal jawabannya ADA di dokumen dan retrieval terbukti menemukannya (eval retrieval mencatat recall 100% untuk pertanyaan yang sama). Mode kepatuhan ketat tampaknya terlalu berhati-hati. Kedua angka ini kini punya alat ukurnya, jadi perbaikannya bisa DIBUKTIKAN, bukan diyakini — jalankan `npm run eval:policy` sebelum dan sesudah. Catatan membaca angkanya: temperature 0,2 berarti hasilnya bergoyang antar jalan (dua jalan berturut mencatat 9/12 lalu 8/12), jadi selisih satu pertanyaan belum bukti apa-apa.' },
