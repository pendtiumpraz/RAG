...
- **NEVER run `db:push` against the production DB.** `drizzle-kit push` reconciles the DB to `schema.ts` and silently destroys everything the schema doesn't declare. It has burned production THREE times in two days: (1) dropped all partial unique indexes (2026-07-27, broke the usage upsert); (2) DISABLED row level security on every tenant table (2026-07-28 — tenant isolation was OFF until smoke caught it); (3) dropped ALL RLS policies (same day — default-deny, app dead). Mitigations now in `schema.ts`: every raw-SQL index is declared (same name; `uniqueIndex().where()` for partial), and every tenant table ends with `.enableRLS()`. But policies can't be fully declared there, so the rule stands: **production schema changes go through `migrations/*.sql` only** (guarded/idempotent, run with the admin role). `db:push` is for fresh/dev databases and CI (where it runs before `db:migrate` on an empty DB). Recovery from any push accident: `npm run db:migrate`.
